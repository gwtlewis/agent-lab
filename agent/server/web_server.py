...
Protocol (server → client):
  ...
  {"type": "board",     "content": "<html fragment>"}
...
from core.agent import IntegratedAgent, LLM_PROVIDER  # noqa: E402
...
_provider = get_provider(LLM_PROVIDER)
...
@app.get("/health")
async def health():
    return {"status": "ok", "provider": LLM_PROVIDER, "model": _MODEL_NAME}
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        agent = IntegratedAgent(provider=LLM_PROVIDER, tools=[make_dashboard_tool()])
    ...
    await ws.send_text(
        AgentEvent(
            type="status",
            content="connected",
            metadata={"provider": LLM_PROVIDER, "model": _MODEL_NAME},
        ).to_json()
    )
